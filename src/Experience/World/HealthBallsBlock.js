...
export default class HealthBallsBlock {
  constructor(healthMaterial, blockPosition, noOfBallToPlace) {
    ...
    const scoreBlock = this.createHealthBallsBlock(noOfBallToPlace, blockPosition);

    // The socreBlock position should always set like below, bcoz group and rigid bodies are two different objects
    this.healthBlockGroup.position.x = this.healthBlockGroup.position.x + blockPosition.x;
    scoreBlock.position.x = scoreBlock.position.x + blockPosition.x;
    
    this.healthBlockGroup.position.z = this.healthBlockGroup.position.z + blockPosition.z;
  ...
  createHealthBallsBlock(size, positionToSetOn) {
    ...
    let mesh = new Mesh(new BoxGeometry(maxX,maxY,maxZ), new MeshBasicMaterial({color: 0xff0000}))
    ...
    scoreCollider.myData = {score: this.creaditPoints, scoreBlock: this.healthBlockGroup}
