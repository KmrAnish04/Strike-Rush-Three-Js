  ...
  registerEvents() {
    window.addEventListener("mousemove", (event) => {
      ...
      this.RigidBodiesArr.forEach((body, index) => {
        if (index > 0) {
          gsap
            .to(this.RigidBodiesArr[index].position, {
              duration: 0.1,
            ...
            .then(
              console.log(
                "hiii",
                this.RigidBodiesArr[index - 1].position,
                this.RigidBodiesArr[index].position
              )
            );
        }
      });
          ...
          case "ramp": {
            for (let i = 0; i < this.RigidBodiesArr.length; i++) {
              gsap.to(this.RigidBodiesArr[i].velocity, {
                ...
                y: 8,
                z: this.RigidBodiesArr[i].velocity.z - 10,
              });
  ...
  update() {
    ...
    if (this.RigidBodiesArr.length) {
      this.camera.position.set(
        0,
        this.RigidBodiesArr[this.RigidBodiesArr.length - 1].position.y + 20,
        this.RigidBodiesArr[this.RigidBodiesArr.length - 1].position.z + 60
      );
      this.camera.lookAt(
        0,
        this.RigidBodiesArr[this.RigidBodiesArr.length - 1].position.y,
        this.RigidBodiesArr[this.RigidBodiesArr.length - 1].position.z
      );
    }
  }
}
